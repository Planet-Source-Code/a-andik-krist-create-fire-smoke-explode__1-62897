...
Sub SmokePointCreate(x As Single, y As Single, Angle As Single, Speed As Single, Turn As Single)
    Dim i As Byte
    ...
    For i = 0 To SmokePointCount
        If SmokePoint(i).Active = True Then
         ...
         '[-------------------------------------------------]
            ...
            RndAngle = Int(Rnd * 45 + 1) + 90
            
            CreateSmoke SmokePoint(i).x, -SmokePoint(i).y, (RndAngle / 1), (RndSpeed / 1), (RndSpeed / 1), 50
        End If
    Next i
End Sub
         ...
         '[-------------------------------------------------]
            ...
            If Smoke(i).TimeKill > Int(Smoke(i).Length / 3) Then
                ...
                With MatOverride
                    ...
                    .dcDiffuse.a = 0.75 - (Smoke(i).TimeKill * MatOverrideCacl)
                End With
            Else
                ' Fire
                With MatOverride
                    .lFlags = D3DRMMATERIALOVERRIDE_DIFFUSE_ALPHAONLY
                    .dcDiffuse.a = 0.75 - (Smoke(i).TimeKill * MatOverrideCacl) * 2
                End With
            End If
